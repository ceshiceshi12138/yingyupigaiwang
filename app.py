...
# Spark X HTTP API 使用 APIPassword 认证（在控制台获取）
APIPassword = "DQwfSffvLcEqYaGlsieH:aBdLgWCskdjtFaZjkWiO"  
...
class XunfeiChatBot:
    def __init__(self):
        # Spark X HTTP API 配置 - 使用 APIPassword 认证
        self.api_password = APIPassword
        self.api_url = "https://spark-api-open.xf-yun.com/x2/chat/completions"
    
    def send_message(self, message):
        """使用HTTP API发送消息到讯飞星火大模型 (Spark X)"""
        try:
            headers = {
            ...
            print(f"发送HTTP请求到讯飞Spark X API...")
