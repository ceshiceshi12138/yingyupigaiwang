...
class XunfeiChatBot:
    def __init__(self):
        ...
        # HTTP API 配置 - Spark X2
        self.http_host = "spark-api-open.xf-yun.com"
        self.http_path = "/x2/chat/completions"
        self.http_url = f"https://{self.http_host}{self.http_path}"
    
    def _create_http_auth_url(self):
        """生成HTTP API鉴权URL - 使用HMAC-SHA256签名"""
        now = datetime.now(timezone.utc)
        date = now.strftime('%a, %d %b %Y %H:%M:%S GMT')
        
        # 构建签名原文
        signature_origin = f"host: {self.http_host}\ndate: {date}\nPOST {self.http_path} HTTP/1.1"
        ...
        return f"{self.http_url}?{urlencode(params)}"
    
    def send_message(self, message):
        """使用HTTP API发送消息到讯飞星火大模型"""
        try:
            auth_url = self._create_http_auth_url()
