...
# 讯飞API配置
APPID = "0f7949c7"
APISecret = "NzEwYzFlNDFjNDRkM2Y2M2UyOWJhOTEy"
APIKey = "0ac47b767419594e98de12407ffd5a66"
...
class XunfeiChatBot:
    def __init__(self):
        ...
        # Spark X2 WebSocket 配置
        self.ws_host = "spark-api.xf-yun.com"
        self.ws_path = "/v2.0/chat"
        self.domain = "x2"
    
    def _create_ws_auth_url(self):
        """生成WebSocket鉴权URL"""
        now = datetime.now(timezone.utc)
        date = now.strftime('%a, %d %b %Y %H:%M:%S GMT')
        
        # 构建签名原文
        signature_origin = f"host: {self.ws_host}\ndate: {date}\nGET {self.ws_path} HTTP/1.1"
        ...
        # 构建authorization
        authorization_origin = f'api_key="{self.api_key}", algorithm="hmac-sha256", headers="host date request-line", signature="{signature_sha_b64}"'
        authorization = b64encode(authorization_origin.encode('utf-8')).decode('utf-8')
        ...
        return f"wss://{self.ws_host}{self.ws_path}?{urlencode(params)}"
    
    def send_message(self, message):
        """使用WebSocket发送消息到讯飞星火大模型"""
        response_text = ""
        error_msg = None
        ...
        def on_message(ws, message):
            ...
            try:
                ...
                print(f"WebSocket收到消息: {json.dumps(data, ensure_ascii=False)[:200]}...")
                
                # 检查错误
                if data.get("header", {}).get("code") != 0:
                    error_code = data.get("header", {}).get("code")
                    error_info = data.get("header", {}).get("message", "Unknown error")
                    error_msg = f"API错误(代码{error_code}): {error_info}"
                    ws.close()
                    return
        ...
        try:
            auth_url = self._create_ws_auth_url()
            print(f"连接到WebSocket: {auth_url[:80]}...")
